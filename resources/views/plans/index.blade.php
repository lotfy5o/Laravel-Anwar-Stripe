                            ...
                            @foreach ($plans as $plan)
                                <div class="text-center flex-grow-1 pt-3 px-lg-4">
                                    ...
                                    <p class="mt-3 d-flex align-items-center justify-content-center">
                                        <span
                                            class="display-5 fw-bold text-dark">{{ $plan->price() }}</span>
                                        <span
                                            class="ms-2 text-muted fs-6">/{{ $plan->interval }}</span>
                                    </p>

                                    <a href="{{ route('direct.subscription', $plan) }}"
                                        class="btn btn-primary mt-3 w-50">
                                        {{ $plan->name == 'monthly-plan' ? 'Try Now' : 'Buy Plan' }}
                                    </a>
                                </div>
                            @endforeach
                        @endif
